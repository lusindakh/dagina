...
function updateDayCounter() {
  const now = Date.now();
  const diffDays = Math.floor((now - START_DATE) / 86400000);
  dayCounterEl.textContent = `${diffDays} дахь өдөр 💞`;
}
...
function updateFullCountdown() {
  ...
  yearsEl.textContent = Math.floor(totalDays / 365);
  monthsEl.textContent = Math.floor((totalDays % 365) / 30);
  daysEl.textContent = (totalDays % 365) % 30;
...
function updateNightMode() {
  ...
  document.body.classList.toggle("night-mode", hour >= 22 || hour < 5);
}
...
function playSong() {
  bgMusic.play().then(() => {
    ...
    vinylLarge.style.animationPlayState = "running";
  }).catch(() => {});
}
...
function pauseSong() {
  ...
  vinylLarge.style.animationPlayState = "paused";
}
